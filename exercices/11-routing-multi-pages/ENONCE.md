...
Construire une application multi-pages avec React Router v7. Tu apprendras a configurer des routes, a naviguer entre les pages, à utiliser les paramètres dynamiques, a proteger des routes et a charger les composants en lazy loading.
...
2. **Créer la structure des pages** dans `src/exercises/ex11/pages/` :
   ...
   - **`TaskDetailPage.tsx`** : detail d'une tache, récupérer l'`id` depuis les paramètres d'URL avec `useParams`. Afficher "Tache introuvable" si l'id n'existe pas.
   - **`AboutPage.tsx`** : page "A propos" avec du contenu statique.
   - **`NotFoundPage.tsx`** : page 404 affichee pour les routes inconnues.

3. **Créer le composant** `src/exercises/ex11/components/Layout.tsx` :
...
4. **Créer le composant** `src/exercises/ex11/components/ProtectedRoute.tsx` :
   - Accepter une prop `isAuthenticated: boolean`.
   - Si authentifie, rendre `children` (où `<Outlet />`).
   - Sinon, rediriger vers la page d'accueil avec `<Navigate to="/" />`.

5. **Créer le fichier** `src/exercises/ex11/router.tsx` :
...
6. **Créer le fichier** `src/exercises/ex11/App.tsx` :
...
- Mode `strict` active.
- Les paramètres d'URL doivent etre types (`useParams<{ id: string }>()`).
...
- [ ] Ajouter un système d'authentification simule (bouton login/logout dans le Layout).
...
| `useParams` récupéré l'id de la tache              | oui     |
