...
Construire une page de dashboard complete avec Tailwind CSS : sidebar responsive (hamburger sur mobile), cartes de statistiques, tableau de taches, et toggle dark mode. Tu vas maîtriser les classes utilitaires de Tailwind, le responsive design avec les breakpoints, et le mode sombre.
...
2. **Sidebar** `src/components/dashboard/Sidebar.tsx` :
   ...
   - Liens de navigation : Dashboard, Taches, Projets, Équipe, Parametres.
...
3. **Cartes de statistiques** `src/components/dashboard/StatsCards.tsx` :
   ...
   - Chaque carte : icone, valeur numérique, label, variation (+/- avec couleur).
...
5. **Dark mode** :
   ...
   - Utiliser la stratégie `class` de Tailwind (`dark:` prefix).
...
- [ ] Supporter la préférence système avec `prefers-color-scheme`.
