...
- [ ] Le `<Profiler>` est integre et les mesures s'affichent dans la console
- [ ] Les mesures "avant" sont documentees (duree de rendu, nombre de re-renders)
- [ ] `useMemo` est utilise pour le filtrage des produits (dependances : `search`, `selectedCategory`)
- [ ] `useMemo` est utilise pour le tri des produits (dependances : `filteredProducts`, `sort`)
- [ ] `React.memo` est applique sur `ProductCard` pour eviter les re-renders inutiles
- [ ] `useCallback` stabilise les handlers passes en props (`handleSelect`, `handleClose`)
- [ ] `React.lazy` + `Suspense` charge le modal de detail a la demande
- [ ] Le modal lazy-loaded a un `export default`
- [ ] Les mesures "apres" montrent une amelioration significative
