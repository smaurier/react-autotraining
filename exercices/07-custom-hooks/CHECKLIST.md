...
- [ ] `useLocalStorage` persiste dans `localStorage` à chaque changement
- [ ] `useLocalStorage` supporte la forme fonctionnelle du setter `(prev) => newValue`
- [ ] `useLocalStorage` utilise un générique `<T>` pour le typage
- [ ] `useDebounce` retourne la valeur avec le delai specifie
- [ ] `useDebounce` nettoie le timeout dans le cleanup de `useEffect`
- [ ] `useDebounce` utilise un générique `<T>` pour le typage
- [ ] `useMediaQuery` utilise `window.matchMedia` pour évaluer la query
