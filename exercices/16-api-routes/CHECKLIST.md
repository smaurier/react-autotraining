...
- [ ] Le store en memoire expose `getTasks`, `getTaskById`, `createTask`, `updateTask`, `deleteTask`
- [ ] GET `/api/tasks` retourne la liste des taches avec un status 200
- [ ] POST `/api/tasks` cree une tache et retourne un status 201
...
- [ ] `useFormStatus` est utilise dans un composant enfant du `<form>` (pas dans le meme composant)
