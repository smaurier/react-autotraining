...
- [ ] L'ajout d'une tache cree un nouvel objet avec `crypto.randomUUID()`
- [ ] Le champ est vide apres l'ajout et l'ajout de texte vide est bloque
- [ ] La bascule du statut utilise `.map()` de maniere immutable
- [ ] La suppression utilise `.filter()` de maniere immutable
