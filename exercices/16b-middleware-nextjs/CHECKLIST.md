...
- [ ] Le `callbackUrl` est sauvegarde en query param et utilise apres la connexion
- [ ] Les headers de securite sont ajoutes (`X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`)
- [ ] Un `X-Request-Id` unique est genere pour chaque requete
...
- [ ] `router.refresh()` est appele apres modification du cookie
